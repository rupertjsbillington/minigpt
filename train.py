...
batch_size = 24              # micro-batch size
block_size = 1024
...
def get_batch(split):
    ...
    if split == 'val':
        # val is a single contiguous .bin file
        ...
        if _train_shards is None:
            ...
            _train_shard_lens = [os.path.getsize(p) // 2 for p in _train_shards]  # uint16 -> tokens
            print(f"found {len(_train_shards)} train shards, "
                  f"{sum(_train_shard_lens):,} tokens total")

        lens = np.array(_train_shard_lens, dtype=np.int64)
        total = int(lens.sum())
        # uniform-over-tokens global sampling
        ...
        # group samples by shard to minimise memmap re-opens
